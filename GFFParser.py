...
def GFFWriter(gtf_content, out_file="out.gff"):
    """
    ...
    for ent1 in gtf_content:
        ...
        for idx, tid in enumerate(ent1['transcripts']):
            ...
            outfh.write('%s\t%s\t%s\t%d\t%d\t.\t%s\t.\tID=%s;Parent=%s\n' \
                % (chr_name, source, t_type, t_start, t_stop, strand, tid[0], ID))

            for ex_cod in ent1["utr5_exons"][idx]:
                outfh.write('%s\t%s\tfive_prime_UTR\t%d\t%d\t.\t%s\t.\tParent=%s\n' \
                    % (chr_name, source, ex_cod[0], ex_cod[1], strand, tid[0]))

            for ex_cod in ent1['cds_exons'][idx]:
                outfh.write('%s\t%s\tCDS\t%d\t%d\t.\t%s\t%d\tParent=%s\n' \
                    % (chr_name, source, ex_cod[0], ex_cod[1], strand, ex_cod[2], tid[0]))

            for ex_cod in ent1["utr3_exons"][idx]:
                outfh.write('%s\t%s\tthree_prime_UTR\t%d\t%d\t.\t%s\t.\tParent=%s\n' \
                    % (chr_name, source, ex_cod[0], ex_cod[1], strand, tid[0]))

            for ex_cod in ent1['exons'][idx]:
                outfh.write('%s\t%s\texon\t%d\t%d\t.\t%s\t.\tParent=%s\n' \
                    % (chr_name, source, ex_cod[0], ex_cod[1], strand, tid[0]))
